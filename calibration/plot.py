...
f = open('data.dat', 'rb')
grid_heights = pickle.load(f)
f.close()

print(np.min(grid_heights))

#grid_heights = grid_heights - vxSize*1000
...
error = 0
for i in range(len(realXs)):
    print(str(realZs_2_5[i]) + " :: " + str(grid_heights[int(realYs[i]/vxSize), int(realXs[i]/vxSize)]))
    error = error + abs(realZs_2_5[i] - grid_heights[int(realYs[i]/vxSize), int(realXs[i]/vxSize)])
err2 = error
print(error)

error = 0
for i in range(len(realXs)):
    print(str(realZs_3[i]) + " :: " + str(grid_heights[int(realYs[i]/vxSize), int(realXs[i]/vxSize)]))
    error = error + abs(realZs_3[i] - grid_heights[int(realYs[i]/vxSize), int(realXs[i]/vxSize)])
err3 = error
print(error)

error = 0
for i in range(len(realXs)):
    print(str(realZs_3_5[i]) + " :: " + str(grid_heights[int(realYs[i]/vxSize), int(realXs[i]/vxSize)]))
    error = error + abs(realZs_3_5[i] - grid_heights[int(realYs[i]/vxSize), int(realXs[i]/vxSize)])
err4 = error
print(error)

plt.figure(figsize=(6.4,4.8))
plt.scatter(x=[2, 2.5, 3, 3.5], y=[err1, err2, err3, err4], color='red')
plt.xlabel("Pressure (psi)", fontsize=18)
plt.ylabel("Error (mm)", fontsize=18)
plt.title("Calibration", fontsize=18)
...
X, Y = np.meshgrid(X, Y)
surf = ax.plot_surface(X, Y, grid_heights, alpha=0.8, label = 'simulation')
...
for i in range(len(realXs)):
    ax.scatter(xs=realXs[i]*1000, ys=realYs[i]*1000, zs=realZs_3[i], color='red')
    #ax.scatter(xs=realXs[i]*1000, ys=realYs[i]*1000, zs=grid_heights[int(realYs[i]/vxSize), int(realXs[i]/vxSize)], color='green')

ax.scatter(xs=realXs[i-1]*1000, ys=realYs[i-1]*1000, zs=realZs_3[i-1], color='red', label='real data')
