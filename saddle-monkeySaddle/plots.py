...
plt.plot(list(range(1, gens+1)), newFitnesses, color='blue', label="error1 (target=monkey saddle)")
...
f.close()
newFitnesses = [x / len(indices) for x in fitnesses]
plt.plot(list(range(1, gens+1)), newFitnesses, color='red', label="error2 (target=simple saddle)")
