...
f.close()
temp = []
with open('Results0/avg_error1.dat', "rb") as f:
    for g in range(1, gens+1):
        try:
            temp.append(pickle.load(f))
        except EOFError:
...
plt.plot(list(range(1, gens+1)), mean_f, color='blue', label="Cylinder", linewidth=2)
...
with open('Results0/avg_error2.dat', "rb") as f:
    for g in range(1, gens+1):
        try:
            temp.append(pickle.load(f))
        except EOFError:
...
mean_f = numpy.mean(fitnesses, axis=0)
std_f = numpy.std(fitnesses, axis=0)
plt.plot(list(range(1, gens+1)), mean_f, color='red', label="Sphere", linewidth=2)
